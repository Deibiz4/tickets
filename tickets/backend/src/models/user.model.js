...
class User {
  ...
  static async findById(id) {
    const result = await query(
      'SELECT id, username, email, full_name, role, created_at FROM tickets.users WHERE id = $1',
      [id]
    );
    return result.rows[0] || null;
  }
  ...
  static async getAllUsers() {
    const result = await query(
      'SELECT id, username, email, full_name, role, created_at FROM tickets.users ORDER BY created_at DESC'
    );
    return result.rows;
  }
